...
import { downloadAndReplaceImages } from "./downImage.js";
...
const main = async () => {
  ...
  console.log(`获取目录成功：完结 ${finishSections.length}章，写作中 ${progressSections.length}章`);
  ...
  for (let i = 0; i < finishSections.length; i++) {
    ...
    const content = await downloadAndReplaceImages(sectionInfo.content, { bookName: bookName, dir: "assets" });
